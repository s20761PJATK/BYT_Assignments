...
public class Money implements Comparable {
	...
	public String toString() {
		String mon = Integer.toString(amount);
		int monlen = mon.length();
		String coins = mon.substring(monlen-2, monlen);
		String dollars = mon.substring(0, monlen-2);
		return dollars + "." + coins + " " + currency.getName();
	}
	...
	public Boolean equals(Money other) {
		double x = amount;
		double y = currency.valueInThisCurrency(other.amount, other.currency);
	...
	public Money add(Money other) {
		int x = amount;
		int y = currency.valueInThisCurrency(other.amount, other.currency);
		Money moreMoney = new Money(x+y, currency);
	 ...
	 */
	public Money negate() {
		Money mon = new Money(-amount, currency);
		return mon;
	}
	...
	public int compareTo(Object other) {
		double x = currency.valueInThisCurrency(amount, currency);
		double y = currency.valueInThisCurrency(((Money)other).amount, ((Money)other).currency);
